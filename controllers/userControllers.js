...
if(u){
 ...
 const newUser = await userss.create({ nombre:name,telefono,correo:email,contrasena:password,aprobado,token});
  res.json({interruptor:'B',email}); 
}
}catch (error){
console.error(error);
...
/////////////////////////////////////////////////
const getPictureIndex = async(req,res)=>{
...
const loginPost = async (req,res)=>{
  try{
    ...
    if(usuario){
       ...
       res.json({interruptor:'aprobado',tokenn:T});
      }
       }else{
        res.json({interruptor:'desaprobado'});
       }
    }else{
      res.json({interruptor:'noRegister'});
    }
...
// Generar un token único
 ...
 res.cookie('securityToken',token,{httpOnly:true,secure:true});
  ...
  const recoveryURL = `${req.protocol}://${req.get('host')}/restablecerRun?token=${token}&email=${email}`;
...
////////////////////////////////////////////
const atenderClientes = (req,res)=>{
res.render('./client/atenderClientes');
}
///////////////////////////////////////////
module.exports={
 ...
 publicarF,atenderClientes,getPictureIndex
}